 ...
 *
 * <p>Il encapsule l'algorithme actuel sans le modifier.
 */
public final class LevelZeroSectorGenerator {
    ...
    public LevelZeroSectorData generate(int sectorX, int sectorZ, long layoutSeed) {
        ...
        applyRectRooms(cells, random, numRooms, roomMin, roomMax, roomMin, roomMax);
        applyPillarRooms(cells, random);
        applyCustomRooms(cells, random);

        return new LevelZeroSectorData(cells, sectorCols);
    }
    ...
    private void applyPillarRooms(boolean[] cells, Random random) {
    ...
    private void applyCustomRooms(boolean[] cells, Random random) {
